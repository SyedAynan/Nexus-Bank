...
class LoanScoringEngine:
    ...
    def batch_score(self, applications):
        ...
        for app in applications:
            try:
                r = self.score_loan(
                    app['account_id'], float(app['amount']),
    ...
    def compare_applications(self, app_list):
        ...
        factor_keys = ['credit_score', 'account_behavior', 'balance_health',
                       'lti_ratio', 'purpose_risk', 'tenure', 'network_risk']
        return {
            'applications': [{
            ...
            } for s in scored if 'error' not in s],
            'factor_labels': ['Credit', 'Behavior', 'Balance', 'LTI', 'Purpose', 'Tenure', 'Network']
        }
